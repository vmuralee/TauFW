...
class ModuleTauTauSimple(Module):
  ...
  def analyze(self, event):
    ...
    for tau in Collection(event,'Tau'):
      ...
      taus.append(tau)
    if len(taus)!=2: return False
    self.cutflow.Fill(self.cut_tau)
    
    # PAIR
    tau1  = max(taus,key=lambda p: p.pt)
    tau2  = min(taus,key=lambda p: p.pt)
    if tau1.DeltaR(tau2)<0.4: return False
    self.cutflow.Fill(self.cut_pair)
    
    # SAVE VARIABLES
    self.pt_1[0]   = tau1.pt
    self.eta_1[0]  = tau1.eta
    self.q_1[0]    = tau1.charge
    self.id_1[0]   = tau1.idDeepTau2017v2p1VSjet
    self.iso_1[0]  = tau1.rawIso
    self.pt_2[0]   = tau2.pt
    self.eta_2[0]  = tau2.eta
    self.q_2[0]    = tau2.charge
    self.id_2[0]   = tau2.idDeepTau2017v2p1VSjet
    self.iso_2[0]  = tau2.rawIso
    self.m_vis[0]  = (tau1.p4()+tau2.p4()).M()
    self.tree.Fill()
